...
class Home extends Component {
  ...
  handleSubmit = event => {
    ...
    const {taskInput, selectedTag, isTagInputVisible} = this.state
    if (taskInput !== '') {
      const newTask = {
      ...
      this.setState(prevState => ({
        ...
        taskInput: '',
        selectedTag: isTagInputVisible ? selectedTag : '', // Only reset if tag input is visible
        isTagInputVisible: false,
      }))
    }
  }
  ...
  render() {
    ...
    const tagsJSX = tagsList.map(tag => (
      <TagsList
        key={tag.optionId}
        tag={tag}
        active={selectedTag === tag.optionId}
